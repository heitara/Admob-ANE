	...
	<xsl:template name="classHeader">
		...
		<div class="MainContent">
			...
			<table class="classHeaderTable" cellpadding="0" cellspacing="0">
                ...
                <xsl:if test="$config/source">
                    <xsl:variable name="extension" select="'.as'"/>
                    <xsl:variable name="file" select="concat($config/source/@base,translate(@fullname,'.:','//'),$extension)"/>
					<tr>
						<td class="classHeaderTableLabel">Source</td>
						<td>
                            <a href="{$file}">
