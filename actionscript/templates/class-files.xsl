...
<!-- 						<xsl:if test="includeExamples/includeExample/codepart">
        ...
        <redirect:write select="'class.dot'">
            <xsl:output method="text"/>
            <xsl:text>//GraphVis .dot file show class inheritance&#xa;</xsl:text>
            <xsl:text>strict digraph classes {&#xa;</xsl:text>
            <xsl:text>graph [URL=""];&#xa;</xsl:text>
            <xsl:for-each select="//asClass">
                <xsl:variable name="nodeName" select="translate(@fullname,'.$:','___')"/>
                ...
                <xsl:if test="@type='interface'">
                    <xsl:text> [style=dotted]</xsl:text>
                </xsl:if>
                <xsl:text>;&#xa;</xsl:text>
                <xsl:if test="asAncestors/asAncestor">
                    <xsl:variable name="parentName" select="translate(asAncestors/asAncestor[1]/classRef/@fullName,'.$:','___')"/>
                    <xsl:value-of select="$nodeName"/>
                ...
                <xsl:for-each select="asImplements/asAncestor">
                    <xsl:variable name="ancestorName" select="translate(classRef/@fullName,'.$:','___')"/>
                    <xsl:value-of select="$nodeName"/>
                    <xsl:text disable-output-escaping="yes">-&gt;</xsl:text>
...
<!-- 			</em> -->
		...
		<xsl:if test="customs/warning">
			<p><font color="red">Warning: </font><xsl:value-of select="customs/warning/." /></p>
		</xsl:if>
	</xsl:template>
